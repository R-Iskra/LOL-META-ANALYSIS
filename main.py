...
def main():
    ...
    # Initialize Riot client and tracker
    ...
    total_requests = len(puuids) * matches_per_player
    ...
    for player_idx, puuid in enumerate(puuids):
        ...
        for match_idx, match_id in enumerate(match_ids):
            ...
            # Print live ETA on the same line
            print('\r' + ' ' * 150, end='', flush=True)
            print(f"\r{client.tracker.eta_str()} | Player {player_idx+1}/{len(puuids)} | Match {match_idx+1}/{len(match_ids)}", end='', flush=True)
